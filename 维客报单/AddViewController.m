...
- (void)configDetailView {
    ...
    self.userDetailView.scrollEnabled = NO;
    ...
    self.productDetailView = [[ProductDetailView alloc]initWithFrame:CGRectMake(0, 0, Width, (Height - StatusBarAndNavigationBarHeight)*5/12) style:UITableViewStylePlain];
    self.productDetailView.scrollEnabled = NO;
    ...
    self.businessDetailView = [[BusinessDetailView alloc]initWithFrame:CGRectMake(0, 0, Width, (Height - StatusBarAndNavigationBarHeight)*5/12) style:UITableViewStylePlain];
    self.businessDetailView.scrollEnabled = NO;
    ...
    if (self.businessInfoDetailView.bounds.size.height != 0) {
        ...
        if (self.productInfoDetailView.bounds.size.height != 0) {
            ...
            [UIView animateWithDuration:0.4 animations:^{
                ...
                self.businessInfoDetailView.frame = CGRectMake(0, StatusBarAndNavigationBarHeight + (Height - StatusBarAndNavigationBarHeight)*3/12 , Width, (Height - StatusBarAndNavigationBarHeight)*5/12);
...
//    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    ...
    UITextField *otherPhoneTextfield = [self.userDetailView viewWithTag:102];
    ...
    UITextField *buyAddress = [self.productDetailView viewWithTag:104];

    NSString *serviceID = self.businessDetailView.serviceID;
    NSString *serviceName = self.businessDetailView.serivcePro;
    UITextField *faultResult = [self.businessDetailView viewWithTag:102];
    
    UITextField *task = [self.businessDetailView viewWithTag:104];
    
    NSString *time = self.businessDetailView.date;
    if (time.length == 0) {
    ...
    NSDictionary *params = @{
                             ...
                             @"BuyerPhone2":otherPhoneTextfield.text,
    ...
    [NicoNetworking nicoGetWithBaseURL:HomeUrl subURL:@"/Task.ashx?action=addTask" parameters:params success:^(id responseObject) {
