...
@property (nonatomic, strong) AFHTTPSessionManager *checkManager;
...
- (void)requestNetWorking{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/Task.ashx?action=getlist&comid=%@&uid=%@&page=%@&query=%@&state=%@",HomeUrl,@(usermodel.CompanyID),@(usermodel.ID),@(self.page),@"",@"-1"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([detailTaskPlanVC.orderModel.ReceiveTaskStr isEqualToString:@"已查看"]) {
        ...
        if (tableView.tag == 300) {
            ...
            }else if (([((OrderModel *)self.dicList[indexPath.row]).State intValue]==18)) {
                detailTaskPlanVC.theStatus = @"已结单";
            }else {
        ...
        self.checkManager = [AFHTTPSessionManager manager];
        ...
        [self.checkManager POST:URL parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            if (([detailTaskPlanVC.orderModel.UnFinishRemark isEqual:[NSNull null]] ||[detailTaskPlanVC.orderModel.UnFinishRemark isEqualToString:@""])&& ([detailTaskPlanVC.orderModel.CancelReason isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.CancelReason isEqualToString:@""])&& ([detailTaskPlanVC.orderModel.NoEntryReason  isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.NoEntryReason isEqualToString:@""])&&([detailTaskPlanVC.orderModel.Change isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.Change isEqualToString:@""])) {
                ...
                if (([detailTaskPlanVC.orderModel.Change isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.Change isEqualToString:@""])) {
                    detailTaskPlanVC.change = NO;
                }else{
                    detailTaskPlanVC.change = YES;
                }
                
                if (([detailTaskPlanVC.orderModel.UnFinishRemark isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.UnFinishRemark isEqualToString:@""] ) && ([detailTaskPlanVC.orderModel.CancelReason isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.CancelReason isEqualToString:@""]) && ([detailTaskPlanVC.orderModel.NoEntryReason  isEqual:[NSNull null]] || [detailTaskPlanVC.orderModel.NoEntryReason isEqualToString:@""])) {
                    detailTaskPlanVC.cancel = NO;
                }else{
                    detailTaskPlanVC.cancel = YES;
                }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    // 进行网络请求
    UserModel *usermodel = [UserModel readUserModel];
    NSString *urlString = [NSString stringWithFormat:@"%@/Task.ashx?action=getlist&comid=%@&uid=%@&page=%@&query=%@&state=%@",HomeUrl,@(usermodel.CompanyID),@(usermodel.ID),@(self.searchPage),str,@"-1"];
    urlString = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
...
- (void)loadMoreSearchData {
    ...
    UserModel *usermodel = [UserModel readUserModel];
    NSString *urlString = [NSString stringWithFormat:@"%@/Task.ashx?action=getlist&comid=%@&uid=%@&page=%@&query=%@&state=%@",HomeUrl,@(usermodel.CompanyID),@(usermodel.ID),@(self.searchPage),the_TextFiledString,@"-1"];
    urlString = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
